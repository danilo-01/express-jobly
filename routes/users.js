...
const { ensureAdminOrUser, ensureAdmin } = require("../middleware/auth");
const { BadRequestError, NotFoundError } = require("../expressError");
const User = require("../models/user");
const { createToken } = require("../helpers/tokens");
const userNewSchema = require("../schemas/userNew.json");
const userUpdateSchema = require("../schemas/userUpdate.json");
const { checkIfExists } = require("../helpers/sql");
...
router.post("/:username/jobs/:id", async (req, res, next) => {
  try{
    ...
    if(e.code == '23505') next(new BadRequestError("User already applied to this job"))
    next(new BadRequestError("Invalid username/jobId"));
  }
  })
...
router.get("/:username", ensureAdminOrUser, async function (req, res, next) {
  try {
    ...
    const applied = await User.getUserApplications(req.params.username);
